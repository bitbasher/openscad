...
When you open OpenSCAD, you'll see three panel within the window. At the
left is the editor panel is where you'll write code to model 3D objects.
The right panel is where you'll see the 3D rendering of your model. In the
middle there is the preview panel above the Console output panel.

Let's make a tree! Type the following code into the left frame to create
the trunk of the tree:

    cylinder(h = 30, r = 8);

Then preview the model using the menu item Display > F5 preview to show the cylinder.
Now let's represent the leafy part of the tree by a sphere. To do so, we add a
union to combine the trunk with a sphere.

    union() {
      cylinder(h = 30, r = 8); // trunk
      sphere(20);
    }

But, it's not quite right! The leaves are around the base of the
tree. We need it at the top of the trunk. We use a translation before the
sphere to move it 0 distance in X and Y, but up by 40 in Z.
...
And that's it! You made your first 3D model!

There are other primitive shapes (square, cube, and extrudes) that you can
combine using set operations (union, intersection, difference) and 
transform using rotate, scale, and translate to make model anything you
can imagine!

Check out all the other language features in the [OpenSCAD
 ...
 * [graphite2 (1.3.14 -> )](https://github.com/silnrsi/graphite)
